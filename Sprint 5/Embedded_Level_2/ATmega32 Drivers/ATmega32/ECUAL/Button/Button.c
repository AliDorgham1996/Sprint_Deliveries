...
{
	ButtonError_t return_value = BTN_EN_valid_init;
	BTN_State_t   PrevState = BTN_NotPressed; 
	BTN_State_t   CurrentState = BTN_NotPressed; 
	if((BTN_Num >= Btn_Start) && (BTN_Num < Btn_End))
	{
		if(State != NULL)
		{
			Dio_ChannelRead(BTN_Configuration[BTN_Num].Channel, (DioState_t*)&PrevState);
			Delay_Ms(BUTTON_DEBOUNCING_MS);
			Dio_ChannelRead(BTN_Configuration[BTN_Num].Channel, (DioState_t*)&CurrentState);
